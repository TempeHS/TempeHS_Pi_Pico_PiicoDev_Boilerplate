...
class Servo:
    ...
    def __init__(
        ...
        self._max_us = max_us if self._min_us < max_us < period_us else period_us
        self._curr_duty = 0
        self.dead_zone_us = dead_zone_us

    def set_duty(self, duty_us: int):
        duty_us = int(min(max(duty_us, self._min_us), self._max_us))
        self._curr_duty = duty_us
        self.pwm.duty_ns(duty_us * 1000)

    def set_angle(self, angle: int):
        angle = min(max(angle, 0), 180)
        duty_us = int(self._min_us + (angle / 180) * (self._max_us - self._min_us))
